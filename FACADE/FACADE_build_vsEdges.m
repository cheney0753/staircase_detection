...
function vsEdges = FACADE_build_vsEdges(vPts_un, ARGS, noFit)
    ...
    for j=1:nbLine
        ...
        % vPts are NORMALIZED input points
               
        %line fitting and subpixel accuracy
        vL = FACADE_fitL_x(vPts);
        vL = vL/norm(vL(1:2));      
        if ~noFit
            vPts =  FACADE_closest_x_on_l(vL, vPts);    % 'project' points onto lq line fit
            ...
            if isempty(ARGS.mKinv)
                ...
                % after this vPts_un lie on line given by parameters
                % in vL!
            end
        end
        ...
        % why is this?? line fit through the projected points.. 
        % nothing changes here when vPts have been fitted!? why normalize
        % with norm(vL)? i changed to norm(vL(1:2))
%        vL_old = vL
%        vL = FACADE_fitL_x(vPts);
%        vsEdges(j).vL = vL/norm(vL(1:2));   %% Least square line fit on projected normalized point
%        vL_new = vsEdges(j).vL
    vsEdges(j).vL = vL;
     % line parameters for normalized coordinates. tansform into
        % image space by vL' * inv(K)
