function [bestPlane, SupportersID] = fitStairPlane2(points,edgeCandidates,epsilon,trials)
Num_inliersMax = 0;
Num_inliers=0;
loopNO=1;  
...
for n=1:length(edgeCandidates)
    ...
    edgePoints{n}(m,:)=points(edgeCandidates(n).vPts_int(2,m),edgeCandidates(n).vPts_int(1,m),:);
    end
...
if Num_inliersMax==0
    ...
    for n=1:length(edgeCandidates)
        ...
        if NumInEdge/length(edgePoints{n})>0.7;
          SupportersID(k)=n;
          k=k+1;
        end    
    end
